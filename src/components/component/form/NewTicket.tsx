...
function NewTicket() {
  ...
  const notify = (name: string) =>
    toast.success(`Ticket ${name} cadstrado com sucesso`);
  ...
  return (
    <>
      <div
        className={`w-screen bg-opacity-30 backdrop-blur-sm justify-center h-full fixed  ${
          ativo ? "flex" : "hidden"
        }`}
      >
        <form
          onSubmit={handleSubmit(onSubmit)}
          className="flex h-[50vh] mx-auto mt-20 border-blue-300 shadow-md shadow-slate-200 border rounded-md bg-slate-900 w-2/6 flex-col gap-3 items-center justify-center content-center min-h-[50%]"
        >
          <header className="text-white relative -top-6">
            {" "}
            <h1>Novo Ticket</h1>
          </header>
          ...
          <div className="grid grid-cols-1 gap-4 w-11/12 ">
            <div className="space-y-2 flex flex-col ">
              <Label htmlFor="clientId" className="text-white py-2">
                Cliente
              </Label>
              <select
                {...register("clientId", { required: true })}
                id="clientId"
                className={`rounded-md h-11 text-gray-800 p-2 ${
                  errors.clientId && "bg-red-50"
                }`}
              >
                {data?.map((empresa, key) => (
                  <option key={key} value={empresa.id}>
                    {empresa.name}
                  </option>
                ))}
              </select>
          ...
          <div className="grid grid-cols-2 gap-4 w-11/12 ">
            ...
            <div className="space-y-2 flex flex-col ">
              <Label htmlFor="type" className="text-white py-2">
                Tipo
              </Label>
              <select
                defaultValue="ABERTO"
          ...
          <div className="grid gap-4  w-11/12 ">
            <div className="space-y-2 flex flex-col">
              ...
              <textarea
                ...
                id="description"
                className={`rounded-md text-gray-800 p-2 ${
          ...
          <div className="grid grid-cols-2 gap-4  w-11/12 ">
            ...
            <span className="gap-2 flex justify-end mt-4">
              <button
                ...
                className="bg-gray-300 cursor-pointer px-8 py-2 rounded-md w-60"
              >
                Cancelar
              </button>
              <input
                type="submit"
                className="bg-blue-500 px-8 py-2 rounded-md w-60 cursor-pointer"
              />
            </span>
          </div>
        </form>
